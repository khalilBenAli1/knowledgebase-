  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Gestion des Équipes</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-2xl font-bold text-biat-primary dark:text-biat-300">{teamData?.totalManagers || 0}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700">
            <div className="text-sm text-gray-600 dark:text-gray-400">Collaborateurs</div>
            <div className="text-2xl font-bold text-biat-secondary dark:text-gray-300">{teamData?.totalEmployees || 0}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700">
            <div className="text-sm text-gray-600 dark:text-gray-400">Non assignés</div>
            <div className="text-2xl font-bold text-orange-500 dark:text-orange-400">{teamData?.totalUnassigned || 0}</div>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
          <input
            ...
            className="w-full sm:w-96 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-biat-primary focus:border-transparent dark:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-400"
          />
        ...
        {pendingChanges.length > 0 && (
          <div className="mb-6 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg p-4">
            <h3 className="font-semibold text-blue-900 dark:text-blue-200 mb-2">Modifications en attente:</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Managers et leurs équipes</h2>
            ...
            {filtered?.managers.length === 0 && (
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">Aucun manager trouvé</div>
            )}
          </div>

          {/* Unassigned Column */}
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Collaborateurs non assignés</h2>
      ...
      <DragOverlay>
        {activeId ? (
          <div className="bg-white dark:bg-gray-800 border-2 border-biat-primary dark:border-biat-300 rounded-lg p-3 shadow-lg opacity-90">
            <div className="font-semibold dark:text-gray-100">{getDraggingUser()?.name}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">{getDraggingUser()?.email}</div>
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
...
function ManagerCard({ manager, isExpanded, onToggle }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border-2 border-gray-200 dark:border-gray-700 overflow-hidden">
...
function UnassignedZone({ employees }: { employees: User[] }) {
  return (
    <Droppable id="unassigned">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border-2 border-dashed border-orange-300 dark:border-orange-600 p-4 min-h-96">
        {employees.length === 0 ? (
...
function DraggableUser({ user }: { user: User }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-lg p-3 cursor-grab active:cursor-grabbing hover:border-biat-primary dark:hover:border-biat-300 hover:shadow-md transition-all ${
        isDragging ? 'opacity-50' : ''
      }`}
    >
      <div className="font-semibold text-gray-900 dark:text-gray-100">{user.name}</div>
      <div className="text-sm text-gray-600 dark:text-gray-400">{user.email}</div>
      <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">{user.role.name}</div>
    </div>
  );
}
