...
export default function HRFormationRequestsPage() {
  ...
  const getStatusBadge = (status: string) => {
    const styles = {
      manager_approved: 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300',
      MANAGER_APPROVED: 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300',
        ...
        <div className="max-w-6xl mx-auto">
          {requests.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-lg">
              ...
              <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1">Aucune demande</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">Aucune demande en attente de validation RH</p>
            </div>
          ) : (
              ...
              {requests.map((request) => (
                <div key={request.id} className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-lg font-bold text-biat-primary">{request.formation?.title || request.customFormationTitle || 'Formation'}</h3>
                        {!request.formation && (
                  ...
                  {request.managerResponse && (
                    <div className="mb-2 p-2 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded text-xs">
                  ...
                  {reviewingRequest === request.id ? (
                    <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-900 rounded">
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-biat-primary focus:border-transparent resize-none bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
