...
export default function AccueilPage() {
  ...
  return (
    <div className="min-h-screen overflow-y-auto bg-gradient-to-br from-gray-50 to-biat-50">
      <div className="max-w-6xl mx-auto px-3 md:px-4 py-6 md:py-12">
        {/* Hero Section */}
        <div className="text-center mb-8 md:mb-16">
          <div className="inline-block bg-white p-4 md:p-6 rounded-2xl shadow-lg mb-4 md:mb-6">
            <svg className="w-16 h-16 md:w-20 md:h-20 text-biat-primary mx-auto" fill="currentColor" viewBox="0 0 24 24">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-biat-primary mb-3 md:mb-4 px-2">
            Bienvenue sur la plateforme RH
          </h1>
          <h2 className="text-2xl md:text-3xl font-semibold text-biat-secondary mb-6">
            Assurances BIAT
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12">
          ...
          <Link
            to="/chat"
            className="group bg-white rounded-xl shadow-md hover:shadow-xl transition-all p-6 md:p-8 border-2 border-transparent hover:border-biat-primary"
          >
            <div className="bg-biat-100 w-16 h-16 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              <svg className="w-8 h-8 text-biat-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
            </div>
            <h3 className="text-xl font-bold text-biat-primary mb-2">Assistant RH</h3>
            ...
            <div className="flex items-center text-biat-primary font-semibold group-hover:gap-2 transition-all">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-blue-700 mb-2">Les Actualités</h3>
            <p className="text-gray-600 mb-4">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-green-700 mb-2">Les Formations</h3>
            <p className="text-gray-600 mb-4">
        ...
        {!loading && recentActualities.length > 0 && (
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Actualités Récentes</h2>
              <Link to="/actualites" className="text-biat-primary font-semibold hover:text-biat-accent flex items-center gap-1">
                Voir tout
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recentActualities.map((actuality) => (
                <div key={actuality.id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                  {actuality.imageUrl && (
                    <img src={actuality.imageUrl} alt={actuality.title} className="w-full h-40 object-cover" />
                  )}
                  <div className="p-4">
                    <h3 className="font-bold text-lg text-biat-primary mb-2 line-clamp-2">{actuality.title}</h3>
                    <p className="text-gray-600 text-sm line-clamp-3 mb-3">{actuality.description}</p>
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>{new Date(actuality.createdAt).toLocaleDateString('fr-FR')}</span>
                      <Link to="/actualites" className="text-biat-primary font-semibold hover:text-biat-accent">
                        Lire plus →
                      </Link>
                    </div>
                  </div>
        ...
        {!loading && upcomingFormations.length > 0 && (
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to="/formations" className="text-biat-primary font-semibold hover:text-biat-accent flex items-center gap-1">
                Voir tout
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {upcomingFormations.map((formation) => {
                ...
                return (
                  <div key={formation.id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                    {formation.imageUrl && (
                      <img src={formation.imageUrl} alt={formation.title} className="w-full h-40 object-cover" />
                    )}
                    <div className="p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-bold text-lg text-biat-primary line-clamp-1">{formation.title}</h3>
                        <span className={`text-xs font-semibold ${status.color}`}>{status.text}</span>
                      </div>
                      <p className="text-gray-600 text-sm line-clamp-2 mb-3">{formation.description}</p>
                      ...
                      <Link to="/formations" className="mt-3 inline-block text-biat-primary text-sm font-semibold hover:text-biat-accent">
                        En savoir plus →
                      </Link>
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-4 md:p-8 border border-gray-200">
          <h3 className="text-2xl font-bold text-biat-primary mb-6">Accès rapide</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <Link
              to="/chat"
              className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg hover:bg-biat-50 transition-colors group"
            >
              <svg className="w-5 h-5 text-biat-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
              </svg>
              <span className="font-medium text-gray-700 group-hover:text-biat-primary">Poser une question</span>
            </Link>
            {canAccessDocuments && (
              <Link
                to="/documents"
              ...
              >
                ...
                <span className="font-medium text-gray-700 group-hover:text-biat-primary">Consulter les documents</span>
              </Link>
            )}
            <Link
              to="/settings"
            ...
            >
              ...
              <span className="font-medium text-gray-700 group-hover:text-biat-primary">Paramètres</span>
            </Link>
          </div>
        </div>
