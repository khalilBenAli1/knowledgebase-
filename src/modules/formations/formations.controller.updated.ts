...
export class FormationsController {
  ...
  async uploadCatalog(@UploadedFile() file: Express.Multer.File, @Request() req) {
    const result = await this.catalogService.extractAndImport(file.path, req.user.userId);
    return {
  ...
  async extractPreview(@UploadedFile() file: Express.Multer.File, @Request() req) {
    const extracted = await this.catalogService.extractFormationsFromPDF(file.path, req.user.userId);
    return {
